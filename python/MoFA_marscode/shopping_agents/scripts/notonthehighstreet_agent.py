...
from core.web_search.scraper import notonthehighstreet_scraper
...
class Operator:
    ...
    def on_event(
        ...
        if dora_event["type"] == "INPUT":
            if dora_event['id'] == 'notonthehighstreet_search':
                all_results = []
                t1 = time.time()
                send_output("notonthehighstreet_agent_status", pa.array([create_agent_output(step_name='notonthehighstreet_agent_status',
                                                                                    output_data={'agent_name':'notonthehighstreet_agent','agent_status':'Running'},
                ...
                for web_search_text in web_search_tasks:
                    if time.time() - t1 > self.timeout:
                        break
                    try:
                        web_result = notonthehighstreet_scraper(search_keyword=web_search_text,api_key=api_key,)
                        all_results.append(web_result.json())
                    except Exception as e :
                        print(e)
                        continue
                print(" notonthehighstreet_result  :  ",all_results)
                send_output("notonthehighstreet_shopping_result", pa.array([create_agent_output(step_name='notonthehighstreet_shopping_result',
                                                                               output_data=all_results,
                                                                               dataflow_status=os.getenv(
                                                                                   'IS_DATAFLOW_END', False))]),
                            dora_event['metadata'])
                send_output("notonthehighstreet_agent_status", pa.array([create_agent_output(step_name='notonthehighstreet_agent_status',
                                                                                    output_data={'agent_name':'notonthehighstreet_agent','agent_status':'Finish','use_time':time.time()-t1},
                                                                                    dataflow_status=os.getenv(
                                                                                        'IS_DATAFLOW_END', False))]),
                            dora_event['metadata'])
