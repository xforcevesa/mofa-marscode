    ...
    def on_event(
        ...
        if dora_event["type"] == "INPUT":
            ...
            if dora_event["id"] in agent_inputs:
                ...
                send_output("agent_response", pa.array([create_agent_output(step_name='agent_response', output_data=json.dumps(agent_result),dataflow_status=os.getenv('IS_DATAFLOW_END',True))]),dora_event['metadata'])
                print('agent_response:', agent_result)
            elif dora_event["id"] == "feedback":
                send_output("agent_response", pa.array([create_agent_output(step_name='agent_response', output_data='feedback received: ' + dora_event["value"][0].as_py(),dataflow_status=os.getenv('IS_DATAFLOW_END',True))]),dora_event['metadata'])
