    ...
    def on_event(
        ...
        if dora_event["type"] == "INPUT":
            ...
            if flag[-1] in [' ', 'B']:
                ...
                send_output("agent_comm", pa.array([create_agent_output(step_name='agent_response', output_data="BA " + agent_result,dataflow_status=os.getenv('IS_DATAFLOW_END',True))]),dora_event['metadata'])
                print('agent_response:', agent_result)
            elif flag[-1] == '^':
                send_output("agent_response", pa.array([create_agent_output(step_name='agent_response', output_data='BA ' + 'task received: ' + text,dataflow_status=os.getenv('IS_DATAFLOW_END',True))]),dora_event['metadata'])
